...
def recbole_run(model, dataset):
    ...
    parser.add_argument('--config_files', '-c', type=str, default='current_lightgcn.yaml', help='config files')
    ...
    for weight_decay in [0.0001, 0.001, 0.01]:
        for n_layers in [1,2,3,4]:
            ...
            if not check_params(logs, config_dict):
                ...
                shutil.copy('confs/conf_lightgcn.yaml', 'current_lightgcn.yaml')
                with open('current_lightgcn.yaml', 'a') as f:
                    f.write(f'\nweight_decay: {weight_decay}\nn_layers: {n_layers}')
                recbole_run(model, dataset)
                os.remove('current_lightgcn.yaml')
