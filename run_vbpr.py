...
def recbole_run(model, dataset):
    ...
    parser.add_argument('--config_files', '-c', type=str, default='current_vbpr.yaml', help='config files')
...
if __name__ == '__main__':
    ...
    for weight_decay in [0.0001, 0.001, 0.01]:
        ...
        if not check_params(logs, config_dict):
            ...
            shutil.copy('confs/conf_vbpr.yaml', 'current_vbpr.yaml')
            with open('current_vbpr.yaml', 'a') as f:
                f.write(f'\nweight_decay: {weight_decay}')
            recbole_run(model, dataset)
            os.remove('current_vbpr.yaml')
