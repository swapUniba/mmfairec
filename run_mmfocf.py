...
def recbole_run(model, dataset):
    ...
    parser.add_argument('--config_files', '-c', type=str, default='current_mmfocf.yaml', help='config files')
...
if __name__ == '__main__':
    ...
    fair_objective choice ['value', 'absolute', 'under', 'over', 'nonparity', 'none']
        ...
        for fair_weight in [0.0001, 0.001, 0.01]:
            for weight_decay in [0.0001, 0.001, 0.01]:
                ...
                if not check_params(logs, config_dict):
                    ...
                    shutil.copy('confs/conf_mmfocf.yaml', 'current_mmfocf.yaml')
                    with open('current_mmfocf.yaml', 'a') as f:
                        f.write(f'\nfair_objective: {fair_objective}\nfair_weight: {fair_weight}\nweight_decay: {weight_decay}')
                    recbole_run(model, dataset)
                    os.remove('current_mmfocf.yaml')
