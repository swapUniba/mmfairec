...
def recbole_run(model, dataset):
    ...
    parser.add_argument('--config_files', '-c', type=str, default='current_freedom.yaml', help='config files')
...
if __name__ == '__main__':
    ...
    n_layers choice [1,2,3,4]    # for both UI and II matrices
    knn choice [10,20]
    '''
        ...
        for n_layers in [1,2,3,4]:
            for knn in [10,20]:
                ...
                config_dict = {
                    ...
                    'n_ui_layers': n_layers,
                    'n_mm_layers': n_layers,
                    'knn_k': knn,
                ...
                if not check_params(logs, config_dict):
                    ...
                    shutil.copy('confs/conf_freedom.yaml', 'current_freedom.yaml')
                    with open('current_freedom.yaml', 'a') as f:
                        f.write(f'\nweight_decay: {weight_decay}\nn_layers: {n_layers}')
                    recbole_run(model, dataset)
                    os.remove('current_freedom.yaml')